...
df_ajustada <- 
  df |> 
  rename(sexo = sexo_0f_1m,
         classe = classificacao_1_sobrepeso_2_obesidade_3_sarco) |> 
  mutate(sexo = if_else(sexo == 0, "female", "male"),
         classe = case_when(classe == 1 ~ "sobrepeso",
                            classe == 2 ~ "obeso",
                            classe == 3 ~ "sarcopenia"),
         sono_classe = if_else(global_score > 5, "Mau dormidor", "Bom dormidor")
)
```
...
df_ajustada |>  
  mutate(
    z_score_hg = (hg-mean(hg))/sd(hg),
    e_outlier_hg = dplyr::if_else(abs(z_score_hg) > 2, "É outlier", "Não é outlier")
  ) |> 
  ggplot2::ggplot(ggplot2::aes(x = sono_classe,
                               y = hg, 
                               color = e_outlier_hg)) +
...
```{r warning=FALSE}
t.test(hg ~ sono_classe, paired = FALSE,data = df1) |> 
  report()
```
...
df |> 
  group_by(sono_classe) |> 
  summarise(média = mean(handgrip_imc, na.rm = TRUE),
            DP = sd(handgrip_imc, na.rm = TRUE))
```

```{r warning=FALSE}
t.test(handgrip_imc ~ sono_classe, paired = FALSE,data = df) |> 
  report()
```
